...
namespace MAGE
{
	VBuffer::VBuffer(const GraphicsBufferDesc& desc, VDevice* pDevice) : GraphicsBuffer(desc, pDevice), mBuffer(VK_NULL_HANDLE), mDevice(pDevice->GetVkDevice())
	{
		...
		Helpers::MageAssert(vkCreateBuffer(mDevice, &bufferInfo, nullptr, &mBuffer) == VK_SUCCESS, "VBuffer", "Failed to create buffer!");
		...
		Helpers::MageAssert(vkBindBufferMemory(mDevice, mBuffer, memPtr->GetVkDeviceMemory(), alignedOffset) == VK_SUCCESS, "VBuffer", "Failed to bind buffer memory!");
