...
namespace MAGE
{
	...
	VulkanDescBuffer::VulkanDescBuffer(VulkanDescLayout* layout, VulkanDevice* device) : m_device(device->GetDevice()), 
		m_physicalDevice(device->GetAdapter()), m_layout(layout->GetLayout()), m_totalSize(0), m_offset(0)
	{
		// Why the fuck do I need this? There is a function for it already.
		PFN_vkGetDescriptorSetLayoutSizeEXT vkGetDescriptorSetLayoutSizeEXT =
			(PFN_vkGetDescriptorSetLayoutSizeEXT)vkGetDeviceProcAddr(m_device, "vkGetDescriptorSetLayoutSizeEXT");
		PFN_vkGetDescriptorSetLayoutBindingOffsetEXT vkGetDescriptorSetLayoutBindingOffsetEXT =
			(PFN_vkGetDescriptorSetLayoutBindingOffsetEXT)vkGetDeviceProcAddr(m_device, "vkGetDescriptorSetLayoutBindingOffsetEXT");
		...
		vkGetDescriptorSetLayoutSizeEXT(m_device, m_layout, &memorySize);

		// Align memory size
		VkDeviceSize offsetSize;
		memorySize = AlignDescBuffer(memorySize, descriptorBufferProperties.descriptorBufferOffsetAlignment);
		vkGetDescriptorSetLayoutBindingOffsetEXT(m_device, m_layout, 0u, &offsetSize);
		...
		ErrorUtils::VkAssert(vkCreateBuffer(m_device, &bufferInfo, nullptr, &m_buffer), "VulkanDescBuffer");
		...
		ErrorUtils::VkAssert(vkAllocateMemory(m_device, &allocInfo, nullptr, &m_memory), "VulkanDescBuffer");
		ErrorUtils::VkAssert(vkBindBufferMemory(m_device, m_buffer, m_memory, 0), "VulkanDescBuffer");
