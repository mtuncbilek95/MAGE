...
namespace MAGE
{
	...
	bool PlatformFile::Exists(const String& path)
	{
		...
		DWORD attribute = GetFileAttributesA(path.CharString());
	...
	{
		// Create the file
		HANDLE hFile = CreateFileA(path.CharString(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);

		// Check if the file was created
		if (hFile == INVALID_HANDLE_VALUE || hFile == NULL)
		{
			// Log the error
			spdlog::warn("Failed to create the file: {}", std::string_view(ErrorUtils::Handler::GetErrorString()));
			return false;
		}
	...
	bool PlatformFile::Read(const String& path, OwnedBuffer& buffer, bool nullTerminated, const usize start, const usize end)
	{
		...
		// Open the file
		HANDLE hFile = CreateFileA(path.CharString(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		if (hFile == INVALID_HANDLE_VALUE || hFile == NULL)
		{
			spdlog::warn("Failed to read the file: {}", std::string_view(ErrorUtils::Handler::GetErrorString()));
			return false;
		}
	...
	bool PlatformFile::Read(const String& path, String& buffer, const usize start, const usize end)
	{
		...
		// Open the file
		HANDLE hFile = CreateFileA(path.CharString(), GENERIC_READ, NULL, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if (hFile == INVALID_HANDLE_VALUE || hFile == NULL)
		{
			spdlog::warn("Failed to read the file: {}", std::string_view(ErrorUtils::Handler::GetErrorString()));
			return false;
	...
	bool PlatformFile::Write(const String& path, RawBuffer& buffer, const usize startOffset)
	{
		...
		// Check if the file exists
		if (PlatformFile::Exists(path))
			hFile = CreateFileA(path.CharString(), GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		else
			hFile = CreateFileA(path.CharString(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		{
			// Log the error
			spdlog::warn("Failed to write to the file: {}", std::string_view(ErrorUtils::Handler::GetErrorString()));
	...
	bool PlatformFile::Write(const String& path, const String& buffer, const usize startOffset)
	{
		...
		// Write the data to the file
		DWORD bytesWritten;
		if (WriteFile(hFile, buffer.CharString(), buffer.Size(), &bytesWritten, NULL) == 0)
		{
			// Log the error
			spdlog::warn("Failed to write to the file: {}", std::string_view(ErrorUtils::Handler::GetErrorString()));
	...
	bool PlatformFile::Rename(const String& oldPath, const String& newPath)
	{
		...
		if (MoveFileA(oldPath.CharString(), newPath.CharString()) == 0)
		{
			spdlog::warn("Failed to move the file: {}", std::string_view(ErrorUtils::Handler::GetErrorString()));
			return false;
		}
	...
	bool PlatformFile::Copy(const String& oldPath, const String& newPath)
	{
		...
		if (CopyFileA(oldPath.CharString(), newPath.CharString(), true) == 0)
		{
			spdlog::warn("Failed to copy the file: {}", std::string_view(ErrorUtils::Handler::GetErrorString()));
			return false;
		}
	...
	{
		if (DeleteFileA(path.CharString()) == 0)
		{
			spdlog::warn("Failed to delete the file: {}", std::string_view(ErrorUtils::Handler::GetErrorString()));
			return false;
		}
		return true;
	}
