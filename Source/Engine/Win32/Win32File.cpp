...
namespace MAGE
{
	...
	b8 Win32File::Copy(const String& source, const String& destination, bool bOverwrite)
	{
		if (!Exists(source))
		{
		...
		if (CopyFileA(source.c_str(), destination.c_str(), bOverwrite) == 0)
		{
	...
	b8 Win32File::Read(const String& path, String& outFile, bool nullTerminate)
	{
		// Check if the file exists
		if (!Exists(path))
		{
	...
	b8 Win32File::Read(const String& path, MemoryOwnedBuffer& outFile, bool nullTerminate)
	{
		// Check if the file exists
		if (!Exists(path))
		{
	...
	b8 Win32File::Write(const String& path, const MemoryBuffer& data, const u64 offset)
	{
		HANDLE hFile;
	...
	u64 Win32File::GetSize(const String& path)
	{
		// Check if the file exists
		if (!Exists(path))
		{
