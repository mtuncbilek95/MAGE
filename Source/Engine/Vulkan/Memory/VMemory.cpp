...
namespace MAGE
{
	...
	{
		VkMemoryAllocateFlagsInfo allocFlagInfo = { VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_FLAGS_INFO };
		...
		ErrorUtils::VkAssert(vkAllocateMemory(device->GetVkDevice(), &allocInfo, nullptr, &m_memory), "VMemory");

		if (desc.memoryType & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)
			ErrorUtils::VkAssert(vkMapMemory(device->GetVkDevice(), m_memory, 0, VK_WHOLE_SIZE, {}, reinterpret_cast<void**>(&m_mappedData)), "VMemory");
	...
	void VMemory::Destroy()
	{
		...
		{
			vkUnmapMemory(m_rootDevice->GetVkDevice(), m_memory);
			vkFreeMemory(m_rootDevice->GetVkDevice(), m_memory, nullptr);
